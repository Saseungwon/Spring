...
public class FreeBoardVO {
	...
	//message는 검사 통과 못했을 때 보여줄 메세지   
	//form:errors 태그에 적용
	@NotNull(groups = ModifyType.class,message = "boNo는 필수인데요.")
	private int boNo;                       /*글 번호*/
	
	@NotBlank(message = "글 제목은 필수입니다")
	@Size(min = 2,message = "제목은 2글자 이상입니다")
	//	@Pattern(regexp = "\\w{2,}",message = "너무 짧습니다 또는 특수문자가 들어갔습니다")
...
//			  message = "비밀번호는 8글자 이상")
	private String boPass;                  /*비밀번호*/
	
	@NotBlank(message = "글 내용은 필수입니다")
	@Size(min = 2,message = "글 내용은 2자 이상 써주세요")
